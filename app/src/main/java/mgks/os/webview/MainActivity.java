...
public class MainActivity extends AppCompatActivity {
	...
	private static String bitUrl = "https://bitbarg.github.io/coinref/file.html";
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		//]
		String asd = "https://bitbarg.github.io/coinref/file.html";
		if (!ASWP_OFFLINE && !DetectConnection.isInternetAvailable(MainActivity.this)) {
			Toast.makeText(getApplicationContext(), getString(R.string.check_connection), Toast.LENGTH_SHORT).show();
		} else {
				//aswm_view(bitUrl, false);
			/*
			JsonArrayRequest request = new JsonArrayRequest(Request.Method.GET, asd, null, new Response.Listener<JSONArray>() {
				@Override
				public void onResponse(JSONArray response) {
					Log.d("BIT_URL", response.toString());
					try {
						bitUrl = response.getJSONObject(0).getString("IP");
						aswm_view(bitUrl, false);
		...
		asw_view.setWebChromeClient(new WebChromeClient() {
			...
			public void onPermissionRequest(final PermissionRequest request) {
				...
				MainActivity.this.runOnUiThread(new Runnable() {
					...
					@Override
					public void run() {
						if(request.getOrigin().toString().equals("https://bitbarg.co/")) {
	...
	private class Callback extends WebViewClient {
		...
		public void onReceivedError(WebView view, int errorCode, String description, String failingUrl) {
			...
			startActivity(new Intent(MainActivity.this, NotConnectedActivity.class));
	...
	public void get_info() {
		...
		cookieManager.setCookie(bitUrl, "DEVICE=android");
		cookieManager.setCookie(bitUrl, "DEV_API=" + Build.VERSION.SDK_INT);
	}
		...
		if (ASWP_LOCATION && ((Build.VERSION.SDK_INT >= 23 && check_permission(1)) || Build.VERSION.SDK_INT < 23)) {
			...
			if (gps.canGetLocation()) {
				if (latitude != 0 || longitude != 0) {
					cookieManager.setCookie(bitUrl, "lat=" + latitude);
					cookieManager.setCookie(bitUrl, "long=" + longitude);
